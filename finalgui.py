...
class Register():
    def __init__(self, root):
        ...
        # Verify button
        verify_data = Button(btn_frame, command=self.verify_data, text='Verify', font=('times new roman', 16, 'bold'), width=12, cursor='hand2', bg='blue', fg='white')
        ...
        # Delete button
        delete_data = Button(btn_frame, command=self.delete_data, text='Delete', font=('times new roman', 16, 'bold'), width=12, cursor='hand2', bg='blue', fg='white')
        delete_data.grid(row=1, column=0, padx=5, sticky=W)

        # Update button
        update_data = Button(btn_frame, command=self.update_data, text='Update', font=('times new roman', 16, 'bold'), width=12, cursor='hand2', bg='blue', fg='white')
        update_data.grid(row=1, column=1, padx=5, sticky=W)

        #retrieve data button
        retrieve_data = Button(btn_frame, command=self.retrieve_data, text='Retrieve', font=('times new roman', 16, 'bold'), width=12, cursor='hand2', bg='blue', fg='white')
    ...
    def update_data(self):
        ...
        try:
            ...
            if selected_student_id:
                # Corrected the SQL query by replacing '.' with ','
                my_cursor.execute(
                    'UPDATE student_details SET Student_Name=%s, Email_id=%s, Phone_No=%s, Gender=%s, Select_Branch=%s, fees=%s, Exam_result=%s WHERE Student_ID_No = %s',
